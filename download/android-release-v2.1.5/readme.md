...
## 远程Debug模式
使用Debug可以实时远程查看上传的debug数据，便于测试，同时避免debug数据写入线上数据库
...
```java
/**
 * 开启远程调试模式，数据上报至Debug服务器
 *
 * @param debug
 */
public DATracker enableRemoteDebug(boolean debug)
```
	
使用`Debug`模式需通过扫码二维码唤醒App，因此用户需在App入口Activity中添加app的scheme为：**hubble.sdk**

```java
<data
    android:host="hubble.sdk"
    android:scheme="hubble"/>
```

在App入口Activity的`onCreate`和`onNewIntent`中解析Intent，当 Intent 的 Host 为 **hubble.sdk**时，调用 SDK 的 startRemoteDebug 传入 intent 实例。实例代码如下：
...
private void parseIntent(Intent intent) {
    if (null != intent) {
        ...
        if (null != uri) {
            if ("hubble.sdk".equals(uri.getHost())) {
...
开启控制台日志。建议在自定义`Application`的 onCreate 中调用。
	
	DATracker.getInstance().enableLog(true);
	
关闭控制台日志。建议在自定义`Application`的 onCreate 中调用。
